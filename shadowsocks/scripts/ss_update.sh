...
main_url="https://raw.githubusercontent.com/moetaku/fancyss_arm/master"
backup_url=""
...
update_ss(){
	...
	if [ -n "$ss_basic_version_web1" ];then
		...
		echo_date 主服务器在线版本号："$ss_basic_version_web1" 和本地版本号："$ss_basic_version_local" 不同！
			...
			curl -o shadowsocks.tar.gz "$main_url"/shadowsocks.tar.gz
...
update_ss2(){
	...
	echo_date "下载地址：https://github.com/moetaku/fancyss_arm"
	echo_date "请下载shadowsocks.tar.gz文件，使用离线安装进行安装"
	sleep 1
	exit
}
